...
class TXFIFO1(width: Int, depth: Int) extends RawModule {
  val io = IO(new Bundle {
    ...
    val writePtr = Output(UInt((3 + 1).W))
    // read-domain
    val dataOut = Output(UInt(width.W))
    val readEn = Input(Bool())
    val readClk = Input(Clock())
    val empty = Output(Bool())
    val readPtr = Output(UInt((3 + 1).W))
    // reset
    val systemRst = Input(Bool())
  })
 
  val ram = SyncReadMem(1 << 3, UInt(width.W))   // 2^3
  val writeToReadPtr = Wire(UInt((3 + 1).W))  // to read clock domain
  val readToWritePtr = Wire(UInt((3 + 1).W))  // to write clock domain
 
  // write clock domain
  withClockAndReset(io.writeClk, io.systemRst) {
    val binaryWritePtr = RegInit(0.U((3 + 1).W))
    val binaryWritePtrNext = Wire(UInt((3 + 1).W))
    val grayWritePtr = RegInit(0.U((3 + 1).W))
    val grayWritePtrNext = Wire(UInt((3 + 1).W))
    ...
    fullValue := (grayWritePtrNext === Cat(~grayReadPtrDelay1(3, 3 - 1), grayReadPtrDelay1(3 - 2, 0)))
    isFull := fullValue
 
    when(io.writeEn && !isFull) {
      ram.write(binaryWritePtr(3 - 1, 0), io.dataIn)
    }
  ...
  withClockAndReset(io.readClk, io.systemRst) {
    val binaryReadPtr = RegInit(0.U((3 + 1).W))
    val binaryReadPtrNext = Wire(UInt((3 + 1).W))
    val grayReadPtr = RegInit(0.U((3 + 1).W))
    val grayReadPtrNext = Wire(UInt((3 + 1).W))
    ...
    io.dataOut := ram.read(binaryReadPtr(3 - 1, 0), !isEmpty)
    io.empty := isEmpty
  }  
}
...
object TXFIFO1Gen extends App {
  chisel3.Driver.execute(args, () => new TXFIFO1(17, 2))
}
